...
- Learn basic ways to work with APIs.
...
We'll continue exploring the `requests` library. The project is organized with a
`lib` folder, since we won't be using Flask.
...
The `requests` library lets you write Python code to send HTTP requests.
...
If you run the code, it should output the JSON response from the NYC Open Data
API!

```console
python lib/nyc_api.py
```
...
Copy and paste the following code into the GetPrograms class:
...
Let's `print` out a unique list of the agencies. Paste in the following lines
after the lines you commented out. We will create a set from the resulting list
of agencies to avoid printing duplicates.
...
Now, run the program again with `python lib/nyc_api.py` in your terminal. You
should see the agencies listed something like this:
